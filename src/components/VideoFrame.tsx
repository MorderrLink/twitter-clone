...
function VideoFrame({file}: VideoFrameProps) {
  ...
  return (
    <div className="w-3/5">
        <video onDoubleClick={interactVideoModal} src={file} autoPlay muted loop/>
        ...
        {/* MODAL */}
        <div onClick={() => { setModal(false); setVideoMuted(true)  }} className={`h-full w-full flex top-0 left-0 z-40 fixed items-center justify-center ${modal ? "" : "hidden"} bg-opacity-80 bg-black overflow-y-hidden`}>
          <div className={`w-7/12 h-7/12 absolute flex flex-col-reverse items-center justify-center bg-opacity-95 bg-slate-100`}>
            <video controls className={`w-full h-full`} muted={videoMuted} src={file}> vid </video>
          </div>
        </div>
    </div>
  )
